...
if __name__ == "__main__":
    ...
    train_history = pd.read_csv(
        os.path.join(raw_data_dir, "trainHistory.csv"), index_col="id"
    )
    test_history = pd.read_csv(
        os.path.join(raw_data_dir, "testHistory.csv"), index_col="id"
    )

    train_history.drop("repeattrips", axis=1, inplace=True)
    train_history["TARGET"] = train_history.transform(
        {"repeater": lambda x: True if x == "t" else False}
    )
