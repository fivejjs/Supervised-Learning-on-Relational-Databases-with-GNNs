...
class ScalarQuantileOrdinalEnc(EncBase):
    ...
    def enc_cat(self, data):
        ...
        if None in data:
            ...
            null_idxs = np.where(data is None)[0]
            val_idxs = np.where(data is not None)[0]
            if len(val_idxs) > 0:
...
class ScalarRescaleEnc(EncBase):
    ...
    def enc_cont(self, scalars):
        ...
        null_idxs = np.where(np.array(scalars) is None)[0]
        val_idxs = np.where(np.array(scalars) is not None)[0]
...
class DatetimeScalarEnc(EncBase):
    ...
    def enc_cont(self, datetimes):
        ...
        if type(datetimes[0]) == Date:
            datetimes = [
                datetime(year=d.year, month=d.month, day=d.day) for d in datetimes
            ]
