...
if __name__ == "__main__":
    ...
    app_train = pd.read_csv(
        os.path.join(data_dir, "application_train.csv"), nrows=dp_limit, index_col=False
    )
    app_test = pd.read_csv(
        os.path.join(data_dir, "application_test.csv"), nrows=dp_limit, index_col=False
    )
    bureau = pd.read_csv(
        os.path.join(data_dir, "bureau.csv"), nrows=dp_limit, index_col=False
    )
    bureau_balance = pd.read_csv(
        os.path.join(data_dir, "bureau_balance.csv"), nrows=dp_limit, index_col=False
    )
    cash = pd.read_csv(
        os.path.join(data_dir, "POS_CASH_balance.csv"), nrows=dp_limit, index_col=False
    )
    credit = pd.read_csv(
    ...
    es = es.entity_from_dataframe(
        entity_id="bureau", dataframe=bureau, index="SK_ID_BUREAU"
    )
    es = es.entity_from_dataframe(
        entity_id="previous", dataframe=previous, index="SK_ID_PREV"
    )
    ...
    r_bureaubalance_bureau = ft.Relationship(
        es["bureau"]["SK_ID_BUREAU"], es["bureau_balance"]["SK_ID_BUREAU"]
    )
    r_previousapp_app = ft.Relationship(
        es["app"]["SK_ID_CURR"], es["previous"]["SK_ID_CURR"]
    )
    # r_cashbalance_app = ft.Relationship(es['app']['SK_ID_CURR'], es['cash']['SK_ID_PREV'])
    r_cashbalance_previousapp = ft.Relationship(
        es["previous"]["SK_ID_PREV"], es["cash"]["SK_ID_PREV"]
    )
    # r_credit_app = ft.Relationship(es['app']['SK_ID_CURR'], es['credit']['SK_ID_CURR'])
    r_credit_previousapp = ft.Relationship(
        es["previous"]["SK_ID_PREV"], es["credit"]["SK_ID_PREV"]
    )
    # r_installments_app = ft.Relationship(es['app']['SK_ID_CURR'], es['installments']['SK_ID_CURR'])
    r_installments_previousapp = ft.Relationship(
        es["previous"]["SK_ID_PREV"], es["installments"]["SK_ID_PREV"]
    )
