...
if __name__ == "__main__":
    ...
    projects = pd.read_csv(
        os.path.join(data_dir, "projects.csv"), nrows=dp_limit, index_col=False
    )
    essays = pd.read_csv(
        os.path.join(data_dir, "essays.csv"), nrows=dp_limit, index_col=False
    )
    resources = pd.read_csv(
        os.path.join(data_dir, "resources.csv"), nrows=dp_limit, index_col=False
    )
    outcomes = pd.read_csv(
        os.path.join(data_dir, "outcomes.csv"),
        usecols=["projectid", "is_exciting"],
        nrows=dp_limit,
        index_col=False,
    )
    ...
    # Fix latlong
    projects["school_latlong"] = projects[
    ...
    r_essay_project = ft.Relationship(
        es["project"]["projectid"], es["essay"]["projectid"]
    )
    r_resource_project = ft.Relationship(
        es["project"]["projectid"], es["resource"]["projectid"]
    )
