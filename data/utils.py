        ...
        for node_type, features in db_info["node_types_and_features"].items():
            for feature_name, feature_info in features.items():
                if feature_info["type"] == "CATEGORICAL":
                    query = "MATCH (n:{0}) RETURN DISTINCT n.{1} ORDER BY n.{1} ;".format(
                    ...
                    query = "MATCH (n:{0}) RETURN n.{1} ;".format(
                        node_type, feature_name
                    )
                    print("running: {}".format(query))
                    ...
                    enc.fit(scalars)
                    feature_info["PowerTransformer_lambdas_"] = float(
                        enc.scaler.lambdas_
                    )
                    feature_info["PowerTransformer_scale_"] = float(
                        enc.scaler._scaler.scale_
                    )
                    feature_info["PowerTransformer_mean_"] = float(
                        enc.scaler._scaler.mean_
                    )
                    feature_info["PowerTransformer_var_"] = float(
                        enc.scaler._scaler.var_
                    )
                    feature_info["PowerTransformer_n_samples_seen_"] = int(
                        enc.scaler._scaler.n_samples_seen_
                    )

                    enc = ScalarQuantileTransformerEnc()
                    enc.fit(scalars)
                    feature_info["QuantileTransformer_n_quantiles_"] = int(
                        enc.scaler.n_quantiles_
                    )
                    feature_info[
                        "QuantileTransformer_quantiles_"
                    ] = enc.scaler.quantiles_[:, 0].tolist()
                    feature_info[
                        "QuantileTransformer_references_"
                    ] = enc.scaler.references_.tolist()

                    enc = ScalarQuantileOrdinalEnc()
                    enc.fit(scalars)
                    feature_info["KBinsDiscretizer_n_bins_"] = int(enc.disc.n_bins_)
                    feature_info["KBinsDiscretizer_bin_edges_"] = enc.disc.bin_edges_[
                        0
                    ].tolist()
                ...
                query = "MATCH (n:{0}) WHERE n.{1} IS null RETURN count(n);".format(
                    node_type, feature_name
                )
                print("running: {}".format(query))
...
def build_db_info(db_name, db_info, test_dp_query, train_dp_query):
    db_info_path = os.path.join(
        project_root, "data", db_name, "{}.db_info.json".format(db_name)
    )
...
def create_datapoints_with_xargs(
    db_name, datapoint_ids, base_query, target_dir, n_jobs
):
    """
    ...
    cmd = "cat {} | xargs -I DPID --max-procs={} -n 1 {} -m {} {} DPID '{}' {}".format(
        temp_file,
        n_jobs,
        sys.executable,
...
def get_db_container(db_name):
    ...
    plugin_dir = os.path.join(
        project_root, "neo4j-apoc-plugins"
    )  # These need to be downloaded, see https://github.com/neo4j-contrib/neo4j-apoc-procedures#using-apoc-with-the-neo4j-docker-image
    ...
    container = client.containers.run(
        ...
        ports={
            "7473/tcp": 7473 + ports_offsets_for_dbs[db_name],
            "7474/tcp": 7474 + ports_offsets_for_dbs[db_name],
            "7687/tcp": 7687 + ports_offsets_for_dbs[db_name],
        },
        mounts=[
            docker.types.Mount(target="/data", source=source_dir, type="bind"),
            docker.types.Mount(target="/plugins", source=plugin_dir, type="bind"),
        ],
...
def get_neo4j_db_driver(db_name):
    ...
    try:
        driver = GraphDatabase.driver(
            "bolt://{}:{}".format("localhost", ports_for_dbs[db_name])
        )
        return driver
    except Exception as e:
        print(
            "Is the neo4j database docker container running?  Run data.utils.get_db_container if not."
        )
        raise e
...
def set_entity_variable_types(entity, node_type, db_info):
    ...
    for var_name, var_type in entity.variable_types.items():
        if var_type not in [Index, Id]:
            feat_info = db_info["node_types_and_features"][node_type].get(var_name)
            if not feat_info:
                print(
                    f"make sure {node_type}.{var_name} is set in variable_types, or is an Id"
                )
            else:
...
def run_dfs(
    ...
    dfs_object = ft.DeepFeatureSynthesis(
    ...
    features = [
        f for f in features if not ("TARGET" in f._name and "TARGET" != f._name)
    ]
    assert ["TARGET" in f._name for f in features].count(True) == 1
    dfs_features = ft.calculate_feature_matrix(
        features=features,
        entityset=es,
...
def compute_and_save_dfs_features(
    es, target_entity, db_name, max_depth, n_jobs, chunk_size
):
    ...
    dfs_feat_file = os.path.join(
        data_dir, f"tabular_features_dfs_smaller_{db_name}.pkl"
    )
    dfs_features.to_pickle(dfs_feat_file)
