...
def get_kwargs(db_name):
    ...
    n_layers = 2
    scalar_enc = (
        "ScalarRobustScalerEnc"  # ScalarRobustScalerEnc ScalarQuantileOrdinalEnc
    )
    ...
    kwargs.update(
        ...
        optimizer_kwargs=dict(lr=3e-4, weight_decay=weight_decay,),
        wd_bias=False,
        wd_embed=False,
        wd_bn=False,
    )
    ...
    kwargs["model_kwargs"].update(
        ...
        loss_class_kwargs=dict(weight=None,),
        fcout_layer_sizes=[],
    )
    ######################
    # Readout kwargs
    if readout == "avg":
        ...
        readout_kwargs = dict(n_layers=2, act_name="SELU")
    elif readout == "s2s":
        readout_class_name = "Set2Set"
        readout_kwargs = dict(n_iters=2, n_layers=2)
    elif readout == "std":
        readout_class_name = "SetTransformerDecoder"
        readout_kwargs = dict(p_dropout=p_dropout, num_heads=2, n_layers=2, k=3)
    kwargs["model_kwargs"].update(
        readout_class_name=readout_class_name, readout_kwargs=readout_kwargs
    )
    ...
    for db_name in db_names:
        ...
        for train_test_split in [
            ...
            kwargs["log_dir"] = os.path.join(
                "GNN", db_name, model_class_name, experiment_slug, train_test_split
            )
