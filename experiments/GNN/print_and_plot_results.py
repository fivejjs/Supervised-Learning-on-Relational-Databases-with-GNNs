...
if __name__ == "__main__":
    for ds_names, model_names, expt_root_dir in (
        (single_table_ds_names, tab_model_names, tab_expt_root_dir),
                ...
                for model_name in sorted(os.listdir(ds_dir)):
                    ...
                    for train_test_split in [
                        ...
                        chkpts = [
                            p
                            for p in os.listdir(
                                os.path.join(run_dir, train_test_split, "evaluations")
                            )
                            if chkpt_name in p
                        ]
                        assert len(chkpts) == 1
        ...
        for chkpt_name in ["best_acc", "best_auroc"]:
            ...
            for model_name in model_names:
                model_results = df.loc[
                    (df["model_name"] == model_name) & (df["chkpt_name"] == chkpt_name)
                ]
                agg_results = model_results.groupby("ds_name", as_index=True).agg(
                    {"value": "mean"}
                )
                agg_results["std"] = model_results.groupby(
                    "ds_name", as_index=True
                ).agg({"value": "std"})
            ...
            print(
                table_results.transpose()
                .reset_index()
                .to_latex(
                    escape=False, index=False, formatters={"ds_name": lambda x: x[:15]}
                )
                .replace("_", r"\_")
            )
