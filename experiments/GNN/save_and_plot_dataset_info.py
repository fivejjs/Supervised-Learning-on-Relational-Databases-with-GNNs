...
db_names = ("acquirevaluedshopperschallenge", "homecreditdefaultrisk", "kddcup2014")
...
if __name__ == "__main__":
    ...
    for db_name in db_names:
        ...
        df_graph_info.columns = (
            r"$\log_{10}$ number of nodes",
            r"$\log_{10}$ number of edges",
        )
        df_node_info = np.log10(df_node_info + 1)
        df_node_info.columns = (
            r"$\log_{10}$ In Degree (including self-loops)",
            r"$\log_{10}$ Out Degree (including self-loops)",
        )

        g = sns.jointplot(
            x=df_graph_info.columns[0],
