...
local_docker_image_id = (
    "<the_id_of_the_docker_image_that_you_build_from_docker/whole_project/Dockerfile>"
)


def run_script_with_kwargs(
    script_name,
    kwargs,
    session_name,
    locale="local_tmux",
    ...
    if locale == "local_tmux":
        ...
        subprocess.run(
            "tmux send-keys -t {} 'sudo {} -m {} {}' C-m ".format(
                session_name,
                sys.executable,
                script_name,
                base64.b64encode(pickle.dumps(kwargs)).decode(),
            ),
            shell=True,
            env=os.environ.copy(),
        )
        subprocess.run(
            "tmux send-keys -t {} 'exit' C-m ".format(session_name), shell=True
        )

    elif locale == "local_docker":
        subprocess.run("tmux new-session -d -s {}".format(session_name), shell=True)
        subprocess.run(
            "tmux send-keys -t {} 'nvidia-docker run --privileged {} /bin/bash -c \" {} \" ' C-m ".format(
                session_name, local_docker_image_id, container_cmd
            ),
            shell=True,
            env=os.environ.copy(),
        )
        ...
        job = dict(
            ...
            containerOverrides=dict(
                ...
                memory=mb_memory,
                command=command,
                resourceRequirements=[dict(value=str(n_gpu), type="GPU")]
                if n_gpu
                else [],
            ),
