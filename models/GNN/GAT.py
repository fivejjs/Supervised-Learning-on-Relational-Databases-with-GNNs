...
class GAT(GNNModelBase):
    ...
    def __init__(self, n_heads, residual, **kwargs):
        ...
        ), "hidden_dim needs to be divisible by n_heads for shapes to align"
...
class RelationalGATConv(nn.Module):
    def __init__(
        ...
        self.r_attn_fcs = nn.ModuleList(
            [
                TypeConditionalLinear(in_feats, out_feats, self.num_rels, bias=True)
        ...
        self.out_attn_fcs = nn.ModuleList(
            [
                TypeConditionalLinear(out_feats * 2, 1, self.num_rels, bias=True)
                for _ in range(num_heads)
            ]
        )
...
class ERGATConv(RelationalGATConv):
    def __init__(
        ...
        self.fc = TypeConditionalLinear(
            in_feats, out_feats * num_heads, n_node_types, bias=True
        )
        if residual:
            if in_feats != out_feats:
                self.res_fc = TypeConditionalLinear(
                    in_feats, num_heads * out_feats, n_node_types, bias=True
                )
            else:
                self.res_fc = nn.Identity()
        else:
