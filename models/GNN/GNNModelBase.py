...
class GNNModelBase(nn.Module):
    ...
    def __init__(
        ...
        for node_type, features in self.db_info["node_types_and_features"].items():
            ...
            for feature_name, feature_info in features.items():
                if (
                    "{}.{}".format(node_type, feature_name)
                    != self.db_info["label_feature"]
                ):
                    enc = self.feature_encoders[node_type][feature_name]
                    cat_feat_origin_cards += [
                        (f"{feature_name}_{i}", card)
            ...
            self.node_initializers[node_type] = self.init_model_class(
                writer=writer,
                dataset_name=None,
        ...
        self.readout = readouts.__dict__[readout_class_name](
            hidden_dim=hidden_dim, **readout_kwargs
        )
    ...
    def init_batch(self, bdgl: BatchedDGLGraph, b_features):
        ...
        bdgl.ndata["h"] = torch.empty(
            bdgl.number_of_nodes(), self.hidden_dim, device=b_node_types.device
        )
        bdgl.ndata["h"][:] = np.nan
