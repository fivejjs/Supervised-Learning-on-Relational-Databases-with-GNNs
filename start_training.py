...
def train_epoch(writer, train_loader, model, optimizer, scheduler, epoch):
    ...
    for batch_idx, (input, label) in enumerate(tqdm(train_loader)):
        ...
        if batch_idx == 0:
            writer.add_scalar(
                "Training/{}".format(model.loss_fxn.__class__.__name__),
                loss.item(),
                writer.batches_done,
            )
...
def validate_model(writer, val_loader, model, epoch):
    ...
    with torch.autograd.no_grad():
        ...
        writer.add_scalar(
            "Validation/{}".format(model.loss_fxn.__class__.__name__),
            val_loss,
            writer.batches_done,
        )
...
def plot_validation_info(writer, labels, probs):
    ...
    plt.ylabel("Count")
    writer.add_figure(
        "Validation/True Positive Predictive Distribution",
        plt.gcf(),
        writer.batches_done,
    )
    ...
    plt.ylabel("Count")
    writer.add_figure(
        "Validation/True Negative Predictive Distribution",
        plt.gcf(),
        writer.batches_done,
    )
    sns.distplot(distneg, label="True Negative", norm_hist=False, kde=False, color="y")
    ...
    plt.ylabel("Count")
    writer.add_figure(
        "Validation/Predictive Distribution", plt.gcf(), writer.batches_done
    )
    ...
    lw = 2
    plt.plot(
        fpr, tpr, color="darkorange", lw=lw, label=f"ROC curve (area = {val_auroc})"
    )
    ...
    plt.yticks(
        (0, 0.5, 0.5, 1.5, 2.0), ("", 0, "", 1, "")
    )  # dumb hack for weird sns plotting
...
def train_model(
    ...
    def init_model():
        ...
        if load_model_weights_from:
            state_dict = torch.load(
                load_model_weights_from, map_location=torch.device("cpu")
            )
    ...
    if find_lr:
        ...
        model = init_model()
        optimizer = get_optim_with_correct_wd(
            optimizer_class_name, model, optimizer_kwargs, wd_bias, wd_embed, wd_bn
        )
        good_lr = get_good_lr(
            model, optimizer, train_loader, init_value=1e-7, final_value=1.0, beta=0.98
        )
        ...
        if lr_scheduler_class_name == "CyclicLR":
            ...
            writer.train_kwargs["lr_scheduler_kwargs"].update(
                max_lr=good_lr, base_lr=good_lr / 100
            )
    ...
    try:
        for epoch in tqdm(range(epochs)):
            ...
            if val_auroc is not None and val_auroc > best_auroc:
                ...
                save_model_checkpoint(
                    writer, epoch, model, optimizer, scheduler, chkpt_name="best_auroc"
                )
            ...
            if val_acc is not None and val_acc > best_acc:
                best_acc = val_acc
                save_model_checkpoint(
                    writer, epoch, model, optimizer, scheduler, chkpt_name="best_acc"
                )
            ...
            if val_loss < best_loss:
                best_loss = val_loss
                save_model_checkpoint(
                    writer, epoch, model, optimizer, scheduler, chkpt_name="best_loss"
                )
...
def train_non_deep_model(
    ...
    train_data = lgb.Dataset(
        np.concatenate(
            [
                t.numpy()
                for t in [train_data.cat_data, train_data.cont_data]
                if t is not None
            ],
            axis=1,
        ),
    ...
    raw_val_data = np.concatenate(
        [t.numpy() for t in [val_data.cat_data, val_data.cont_data] if t is not None],
        axis=1,
    )
    ...
    test_data = np.concatenate(
        [
            t.numpy()
            for t in [orig_test_data.cat_data, orig_test_data.cont_data]
            if t is not None
        ],
        axis=1,
    )
    ...
    try:
        ...
        if bst.best_iteration < num_boost_round:
            ...
            Path(os.path.join(writer.log_dir, "finished_all_epochs.info")).touch()
        bst.save_model(
            os.path.join(writer.log_dir, "model_checkpoint_best_val_loss.lgb"),
        ...
        writer.add_hparams(
            format_hparam_dict_for_tb(writer.train_kwargs),
            {
                ...
                "hparam/best_acc": 100
                * (1 - bst.best_score["valid_0"]["binary_error"]),
        ...
        for checkpoint_id in ["best_loss", "best_auroc", "best_acc"]:
            ...
            results_dir = os.path.join(
                writer.log_dir, "evaluations", f"model_checkpoint_{checkpoint_id}"
            )
