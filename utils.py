...
def setup_writer(log_dir, debug_network):
    ...
    if os.path.exists(log_dir) and "debug" not in log_dir:
        raise FileExistsError(
            f"Manually delete {log_dir} if you want to overwrite its contents"
        )
...
def log_param_values(writer, model):
    for name, param in model.named_parameters():
        writer.add_histogram(
            "Parameter Values/{}".format(name), param, writer.batches_done
        )
        if param.requires_grad and param.grad is not None:
            writer.add_histogram(
                "Gradients/{}".format(name), param.grad, writer.batches_done
            )
...
def truncate_graph(
    ...
    for node_type, g_features in features.items():
        n_nodes_this_type = cutoff_node_types.count(
            db_info["node_type_to_int"][node_type]
        )
    ...
    def DGL_collator(datapoints):
        ...
        for node_type, features in b_features.items():
            ...
            for feature_name, feature_values in features.items():
                ...
                if (
                    not feature_values
                ):  # In case there are no nodes of this type in the batch
...
def get_train_val_test_datasets(
    ...
    if dataset_name in [
        ...
        if train_fraction_to_use == 1.0:
            ...
            temp_train_ds, _, _ = get_train_val_test_datasets(
                dataset_name, train_test_split, encoders, train_fraction_to_use=1.0
            )
    ...
    if "weight_decay" in optimizer_kwargs:
        ...
        for n, m in model.named_modules():
            if isinstance(m, torch.nn.Embedding) and not wd_embed:
                ...
                for _np, p in m.named_parameters(recurse=False):
                    if "bias" in _np and not wd_bias:
                        no_wd_params.append(p)
                    else:
                        wd_params.append(p)
        assert len(wd_params) + len(no_wd_params) == len(list(model.parameters()))
        params = [{"params": no_wd_params, "weight_decay": 0.0}, {"params": wd_params}]
    else:
...
def get_dataloader(
    dataset: Union[DatabaseDataset, TabularDataset],
    batch_size,
    ...
    if isinstance(dataset, DatabaseDataset):
        collate_fn = get_DGL_collator(
            dataset.feature_encoders,
            dataset.db_info,
